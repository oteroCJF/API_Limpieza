...
using Limpieza.Service.EventHandler.Commands.CFDIs.NodosXML;
using MediatR;
...
namespace Limpieza.Service.EventHandler.Handlers.CFDIs
{
    public class FacturaCreateEventHandler : IRequestHandler<FacturaCreateCommand, Factura>
    {
        ...
        public async Task<bool> VerificaFacturaServicio(Facturas factura)
        {
            try
            {
                Repositorio facturacion = await _context.Repositorios.SingleOrDefaultAsync(f => f.Id == factura.RepositorioId);

                Contrato contrato = await _context.Contratos.SingleOrDefaultAsync(c => c.Id == facturacion.ContratoId);
